...
import { clamp, curry, filter, head, is, isEmpty, isNil, last, map, reduce, sort, tail, unfold } from 'ramda';
...
export const dustThreshold = (feeRate, change) => Math.ceil((Coin.inputBytes(change) + Coin.outputBytes(change)) * feeRate);
...
export const transactionBytes = (inputs, outputs) => {
  const coinTypeReducer = (acc, coin) => {
    ...
    if (acc[type]) acc[type] += 1;else acc[type] = 1;
    return acc;
  };
...
export const effectiveBalance = curry((feePerByte, inputs, outputs = [{}]) => List(inputs).fold(Coin.empty).overValue(v => clamp(0, Infinity, v - Math.ceil(transactionBytes(inputs, outputs) * feePerByte))));
...
export const descentDraw = (targets, feePerByte, coins, changeAddress) => findTarget(targets, feePerByte, sort((a, b) => a.lte(b), coins), changeAddress);
// ascentDraw :: [Coin(x), ..., Coin(y)] -> Number -> [Coin(a), ..., Coin(b)] -> Selection
export const ascentDraw = (targets, feePerByte, coins, changeAddress) => findTarget(targets, feePerByte, sort((a, b) => b.lte(a), coins), changeAddress);